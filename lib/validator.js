...
Validator = {
  ...
  error: function(rule, key, message, ruleArgs) {
    ...
    return message.replace(_regexp.ruleName, key).replace(_regexp.ruleKey, function(whole, first) {
      if (ruleArgs[first]) {
        return ruleArgs[first];
      } else if (_this.rules[rule][first]) {
  ...
  testInternal: function(obj, rule, key) {
    ...
    if (this.checkRule(theRule)) {
      ...
      if (this.rules[theRule].test.call(context, obj[key], rule)) {
  ...
  validate: function(obj, ruleset) {
    var errors, key, nestedRule, rule, testResult, _i, _len;
    errors = [];
    for (key in ruleset) {
      rule = ruleset[key];
...
Validator.addRule('maxLength', {
  ...
  test: function(str, rule) {
    var maxLength;
    maxLength = (rule != null ? rule.maxLength : void 0) || this.maxLenght;
    return this.rules.lengthBetween.test(str, {
      low: 0,
      high: maxLength
    });
